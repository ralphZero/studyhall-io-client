...
const DataContextProvider = ({ children }: DataContextProviderProps) => {
  ...
  useEffect(() => {
    ...
    if(user) {
      fetch(
        `https://apihallify.link/halls?uid=${user?.uid}`
      )
        .then((res) => res.json())
  ...
  const addDataToList = (hall: Hall, callback: () => void) => {
    setIsLoading(true)
    fetch("https://apihallify.link/halls", {
      method: "POST",
      headers: {
  ...
  const createTaskInHall = (hallId: string, task: Task, callback: () => void) => {
    setIsLoading(true);
    fetch(`https://apihallify.link/halls/${hallId}/tasks`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json'
      },
  ...
  const updateTaskInHall = (hallId: string, task: Task) => {
    setIsLoading(true);
    fetch(`https://apihallify.link/halls/${hallId}/tasks/${task.id}`, {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json"
      },
  ...
  const updateDatesInHall = (hallId: string, dates: PlanDate[]) => {
    ...
    fetch(`https://apihallify.link/halls/${hallId}/dates`, {
      method: "PATCH", 
      headers: {
        "Content-Type": "application/json"
      },
  ...
  const deleteHall = (hallId: string) => {
    ...
    fetch(`https://apihallify.link/halls/${hallId}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
      },
