...
const DataContextProvider = ({ children }: DataContextProviderProps) => {
  ...
  useEffect(() => {
    ...
    if(user) {
      fetch(
        `52.90.64.141:4043/halls?uid=${user?.uid}`
      )
        .then((res) => res.json())
  ...
  const createTaskInHall = (hallId: string, task: Task, callback: () => void) => {
    setIsLoading(true);
    fetch(`52.90.64.141:4043/halls/${hallId}/tasks`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json'
      },
  ...
  const updateTaskInHall = (hallId: string, task: Task) => {
    setIsLoading(true);
    fetch(`52.90.64.141:4043/halls/${hallId}/tasks/${task.id}`, {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json"
      },
  ...
  const updateDatesInHall = (hallId: string, dates: PlanDate[]) => {
    ...
    fetch(`52.90.64.141:4043/halls/${hallId}/dates`, {
      method: "PATCH", 
      headers: {
        "Content-Type": "application/json"
      },
  ...
  const deleteHall = (hallId: string) => {
    ...
    fetch(`52.90.64.141:4043/halls/${hallId}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
      },
